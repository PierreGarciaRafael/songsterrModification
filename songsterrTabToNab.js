...
let noteToHalfTone = (note) => {
    return saxNotes.findIndex(e => e == note.toLowerCase());
}
let froteNoOctave = (halfTone ) =>{
    return saxNotes[(halfTone) % 12];   
}
let toFrote = (halfTone, over12) => {
    if (over12) return froteNoOctave(halfTone);
    return froteNoOctave(halfTone).toUpperCase();
}


//noteToHalfTone(e.textContent)}
let getTuning = (line) => {
...
for (var lineIdx = 0; lineIdx < tabLine.length; lineIdx += 1){
    ...
    for (var fretIdx = 0; fretIdx < frets.length; fretIdx += 1){
        const fretNb = frets[fretIdx].textContent.match(fretNotation);
        if (! fretNb) continue;
        const stringNote = getFretString(frets[fretIdx], tuning);

        frets[fretIdx].textContent = 
        frets[fretIdx].textContent.replace(fretNotation, toFrote(fretNb + stringNote, fretNb >= 12));
    }
}
